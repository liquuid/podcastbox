...
def feed_ws(request):
    ...
    for feed in UserProfile.objects.get(user_id=request.user.id).feeds.values():
        #import pdb; pdb.set_trace()
        feeds += " '%s':{ 'id':'%s', 'url':'%s', 'title':'%s'  }," % ( feed['id'], feed['id'], feed['url'], feed['title'] )
    feeds += '}'
...
def user_feed_ws(request):
    ...
    for feed in UserProfile.objects.get(user_id=request.user.id).feeds.values():
        #import pdb; pdb.set_trace()
        feeds += " '%s':{ 'id':'%s', 'url':'%s', 'silent':'%s', 'title':'%s'  }," % ( feed['id'], feed['id'], feed['url'], feed['silent'], feed['title'] )
    feeds += '}'
    return HttpResponse(json.dumps(eval(feeds)))
...
def episodes_time_line(request):
    ...
    for episode in episode_list.order_by('-updated').values():
        episodes += " '%s':{'id':'%s', 'url':'%s', 'updated':'%s','summary':'%s'  , 'title':'%s' , 'feed_id':'%s' }," % ( episode['id'], episode['id'], episode['url'], episode['updated'], episode['summary'], episode['title'],  episode['feeds_id'])
        print episode['id']
